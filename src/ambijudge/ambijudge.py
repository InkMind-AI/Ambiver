...
class AmbiJudge:
    ...
    def __init__(self, perception_kwargs: Optional[Dict[str, Any]]=None, reasoning_kwargs: Optional[Dict[str, Any]]=None, use_hvim: bool=True, voxel_size: float=0.05, K_max: int=16, tau_iou: float=0.25, tau_iom: float=0.6, min_detections: int=2, use_gpu: bool=True, max_memory_gb: float=4.0, scannet_root: Optional[str]=None):
        """
        ...
        if perception_kwargs is None:
            perception_kwargs = {}
        perception_kwargs.update({'use_hvim': use_hvim, 'voxel_size': voxel_size, 'K_max': K_max, 'tau_iou': tau_iou, 'tau_iom': tau_iom, 'min_detections': min_detections, 'use_gpu': use_gpu, 'max_memory_gb': max_memory_gb, 'scannet_root': scannet_root})
    ...
    def process(self, instruction: str, images: List[np.ndarray], cameras: List[CameraParams], detections: List[Detection], birdseye_image: Optional[np.ndarray]=None) -> Dict[str, Any]:
        """
        ...
        parsed_instruction, candidates = self.perception_engine.process(instruction, images, cameras, detections)
        ...
        verdict = self.reasoning_core.process(parsed_instruction, candidates, instruction, images, birdseye_image)
        ...
        if last_dossier is not None:
            dossier_dict = {'instruction': last_dossier.instruction, 'instances': last_dossier.instances, 'meta': last_dossier.meta}
        results = {'instruction': {'raw': instruction, 'parsed': {'target': parsed_instruction.target, 'attributes': parsed_instruction.attributes, 'relations': parsed_instruction.relations, 'action': parsed_instruction.action}}, 'candidates_found': [{'id': candidate.id, 'representative_image': candidate.representative_image, 'representative_bbox': candidate.representative_bbox, 'score': candidate.score, 'detection_count': len(candidate.detections)} for candidate in candidates], 'count': len(candidates), 'verdict': {'label': verdict.label.value, 'types': [t.value for t in verdict.types], 'sources': verdict.sources, 'explanation': verdict.explanation, 'clarify': verdict.clarify}, 'llm_input': {'messages': llm_messages, 'dossier': dossier_dict}, 'llm_raw_response': llm_raw_response}
        return results
