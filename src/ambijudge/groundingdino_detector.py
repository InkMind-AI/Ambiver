...
class GroundingDINOConfig:
    ...
    box_threshold: float = 0.50  # 设置为0.50平衡检测质量和数量
    text_threshold: float = 0.50  # 设置为0.50平衡文本匹配质量
    cpu_only: bool = False
...
class GroundingDINODetector:
    ...
    def _load_model(self):
        ...
        try:
            ...
            self.model = load_model(
                ...
                device='cuda' if not self.config.cpu_only else 'cpu'
            )
            # Force move to GPU if CUDA is available and not CPU-only
    ...
    def detect_objects(self, image: np.ndarray, text_prompt: str, output_dir: str = None) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        ...
        """
        try:
            # Convert numpy array to image format expected by GroundingDINO
            ...
            # Save temporary image file with unique name
            import time
            ...
            try:
                # Run GroundingDINO official demo script
                import subprocess
                cmd = [
                    'python', 
                    os.path.join(self.grounding_dino_root, 'demo', 'inference_on_a_image.py'),
                    '-c', self.config.config_path,
                    '-p', self.config.weights_path,
                    '-i', temp_image_path,
                    '-o', temp_output,
                    '-t', text_prompt,
                    '--box_threshold', str(self.config.box_threshold),
                    '--text_threshold', str(self.config.text_threshold)
                ]
                ...
                result = subprocess.run(
                    ...
                    capture_output=True, 
                    text=True, 
                    cwd=self.grounding_dino_root,
                    env=env,
                    timeout=120  # 120 second timeout for GPU
                )
                ...
                # Clean up temporary output directory if it was created
    ...
    def _detect_batch_optimized(self, images: List[np.ndarray], target_text: str, visualization_dir: str = None) -> List[Detection]:
        ...
        # Resize images to optimal size for GroundingDINO (800x800)
        target_size = 800
        transform = T.Compose([
        ...
        for i, image in enumerate(images):
            ...
            original_shapes.append(image.shape[:2])  # (height, width)
        ...
        for i, img in enumerate(processed_images[:3]):  # Check first 3
        ...
        try:
            # Use GroundingDINO model directly for batch processing
            with torch.no_grad():
                # Prepare text prompt
                text_prompt = target_text.lower().strip()
                
                # Create NestedTensor for GroundingDINO using the proper function
                from groundingdino.util.misc import nested_tensor_from_tensor_list
                
                # Convert batch tensor to list of tensors for NestedTensor creation
                tensor_list = [batch_tensor[i] for i in range(batch_tensor.shape[0])]
                nested_tensor = nested_tensor_from_tensor_list(tensor_list)
                
                # Run model inference with NestedTensor
                ...
                for view_id in range(len(images)):
                    # Extract boxes, logits, and phrases for this view
    ...
    def _detect_sequential_fallback(self, images: List[np.ndarray], target_text: str, visualization_dir: str = None) -> List[Detection]:
        ...
        # Pre-resize images to reduce processing time
        ...
        for image in images:
            ...
            h, w = image.shape[:2]
            max_size = 512  # Smaller than original 800x800
            if max(h, w) > max_size:
        ...
        for view_id, (resized_image, (orig_h, orig_w, scale)) in enumerate(zip(resized_images, original_shapes)):
            if view_id % 5 == 0:  # Progress indicator every 5 images
                print(f"Processing view {view_id}/{len(images)} (image shape: {resized_image.shape})")
            
            try:
                # Disable per-view visualization to avoid creating empty directories
                view_output_dir = None
                
                # Run GroundingDINO detection without per-view visualization
                boxes, logits, phrases = self.detect_objects(resized_image, target_text, view_output_dir)
                
                if len(boxes) > 0:
                    if view_id % 5 == 0:  # Only print details for progress indicators
                ...
                for i, (box, logit, phrase) in enumerate(zip(boxes, logits, phrases)):
                    ...
                    detection = Detection(
                        ...
                        bbox=(x, y, w, h),
                        confidence=float(logit),
                        mask=None  # GroundingDINO doesn't provide masks
                    )
                    all_detections.append(detection)
                    
            except Exception as e:
                print(f"Warning: Detection failed for view {view_id}: {e}")
                # Don't add fake detections - just skip this view
                # The system should handle empty detection lists properly
        
        print(f"Optimized sequential detection completed: {len(all_detections)} total detections")
        return all_detections
    
    def save_annotated_image(self, image: np.ndarray, boxes: np.ndarray, 
                           logits: np.ndarray, phrases: List[str], 
        ...
        try:
            ...
            # Convert boxes to the format expected by annotate function
            # annotate expects boxes in normalized format [0,1] and (x1, y1, x2, y2)
            if len(boxes) > 0:
                # Convert from pixel coordinates to normalized coordinates
                H, W = image.shape[:2]
                boxes_normalized = boxes.copy().astype(float)
                boxes_normalized[:, [0, 2]] /= W  # x coordinates
                boxes_normalized[:, [1, 3]] /= H  # y coordinates
