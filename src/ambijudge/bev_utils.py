"""Load precomputed BEV maps shipped with AmbiVer."""
...
def generate_bev_from_depth_scene(scannet_root, scene_id, bev_dir=None, **kwargs):
    ...
    """
    del scannet_root, kwargs  # kept for API compatibility with the evaluation runner
    ...
    if gray_path is None and color_path is None:
        raise FileNotFoundError(
            f"BEV image not found: {bev_root}/{scene_id}.(png|jpg) "
            f"or {scene_id}_color.(png|jpg). "
            "Set BEV_MAPS_DIR or place maps under ./bev_maps/."
        )
    ...
    if gray_img is None or color_img is None:
        raise FileNotFoundError(
            f"Incomplete BEV image for {scene_id}: gray={gray_path}, color={color_path}"
        )
